/* Script and color picker prefab from Unity Asset Store;
 * Author: Den Molina
 * Modified by: Davide Zhang
 * 
 */
...
public class ColorPickerTriangle : MonoBehaviour {
    ...
    public bool HasIntersectionVRTri(Vector3 collisionPt)
    {

        if (Vector3.Distance(collisionPt, transform.position) > MainRadius)
        {
            return false;
        }
        
        CurLocalPos = transform.worldToLocalMatrix.MultiplyPoint(collisionPt);
    ...
    public bool HasIntersectionVRCircle(Vector3 collisionPt)
    {
